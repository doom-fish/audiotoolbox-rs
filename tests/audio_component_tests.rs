...
fn audio_component_finds_converter_unit() -> Result<()> {
    ...
    assert_eq!(component_description.component_type, AUDIO_COMPONENT_TYPE_FORMAT_CONVERTER);
    assert_eq!(parent.description()?.component_sub_type, AUDIO_UNIT_SUBTYPE_AU_CONVERTER);
    Ok(())
}
