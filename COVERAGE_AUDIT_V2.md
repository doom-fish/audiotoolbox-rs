# audiotoolbox-rs coverage audit v2 (vs MacOSX26.2.sdk)

Continuation of v1 audit: verified that the 300-symbol sample across `AudioToolbox.framework`, `AudioUnit.framework`, and `AVFAudio.framework` remains accurate in MacOSX26.2.sdk. Re-confirmed both EXEMPT symbols (AVAudioFile, AudioComponentGetIcon) still carry their deprecation attributes in 26.2. No new public macOS symbols detected that would change the VERIFIED/GAPS classification. This is a sampled scoreboard, not proof of exhaustive coverage: see the note below and `COVERAGE.md`.
...
| --- | --- | --- | --- | --- |
| `AVAudioFile` | interface | `AVAudioFile.h` | Legacy/deprecated API intentionally excluded from the coverage target. | `API_DEPRECATED(..., macos(10.10, 26.0), ...)` |
